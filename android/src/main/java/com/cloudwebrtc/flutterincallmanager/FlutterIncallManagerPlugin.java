...
public class FlutterIncallManagerPlugin implements MethodCallHandler {
  ...
  private static final String ACTION_HEADSET_PLUG = (Build.VERSION.SDK_INT >= 21) ? AudioManager.ACTION_HEADSET_PLUG : Intent.ACTION_HEADSET_PLUG;
      ...
      noisyAudioReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intent.getAction())) {
            ...
            if(eventSink != null){
              ConstraintsMap params = new ConstraintsMap();
              params.putString("event", "NoisyAudio");
              params.putString("status","noisy");
              eventSink.success(params.toMap());
            }
          }
        }
      ...
      mediaButtonReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          if (Intent.ACTION_MEDIA_BUTTON.equals(intent.getAction())) {
            ...
            if(eventSink != null){
              ConstraintsMap params = new ConstraintsMap();
              params.putString("event","MediaButton");
              params.putString("eventText", keyText);
              params.putInt("eventCode",keyCode);
              eventSink.success(params.toMap());
            }
          }
        }
  ...
  public void onProximitySensorChangedState(boolean isNear) {
    Log.d(TAG, "onProximitySensorChangedState:" + isNear);
    ...
    if(eventSink != null){
      ConstraintsMap params = new ConstraintsMap();
      params.putString("event","Proximity");
  ...
  private class myToneGenerator extends Thread implements MyPlayerInterface {
    ...
    public void run() {
      ...
      if (caller.equals("mBusytone")) {
        ...
        FlutterIncallManagerPlugin.this.stop();
      }
    }
  }
...
// ===== Internal Classes End =====
  ...
  //  ===== Permission Start =====
  //检测是否可以录制权限
  public void checkRecordPermission(Result result) {
    Log.d(TAG, "RNInCallManager.checkRecordPermission(): enter");
    _checkRecordPermission();
    if (recordPermission.equals("unknow")) {
      Log.d(TAG, "FlutterInCallManager.checkRecordPermission(): failed");
      result.error("checkRecordPermissionFailed", "checkRecordPermission failed", null);
    } else {
      result.success(recordPermission);
    }
  }

  //检测摄像头权限
  public void checkCameraPermission(Result result) {
    Log.d(TAG, "FlutterInCallManager.checkCameraPermission(): enter");
    _checkCameraPermission();
    if (cameraPermission.equals("unknow")) {
      Log.d(TAG, "FlutterInCallManager.checkCameraPermission(): failed");
  ...
  private void _checkRecordPermission() {
    ...
    Log.d(TAG, String.format("FlutterInCallManager.checkRecordPermission(): recordPermission=%s", recordPermission));
  }

  private void _checkCameraPermission() {
    cameraPermission = _checkPermission(permission.CAMERA);
    Log.d(TAG, String.format("FlutterInCallManager.checkCameraPermission(): cameraPermission=%s", cameraPermission));
  }
  ...
  public void requestRecordPermission(Result result) {
    ...
    if (!recordPermission.equals("granted")) {
      _requestPermission(permission.RECORD_AUDIO);
  ...
  public void chooseAudioRoute(String audioRoute) {
